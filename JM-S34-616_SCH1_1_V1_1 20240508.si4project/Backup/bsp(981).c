...
//	LL_GPIO_ResetOutputPin(GPIOF, LL_GPIO_PIN_3);
	...
	LL_GPIO_SetPinMode(GPIOA, LL_GPIO_PIN_3, LL_GPIO_MODE_INPUT);//key_dec
	...
	LL_GPIO_SetPinMode(GPIOA, LL_GPIO_PIN_11, LL_GPIO_MODE_INPUT);//music
...
void APP_ConfigureEXTI(void)
{
	...
	NVIC_EnableIRQ(EXTI4_15_IRQn);
		...
		/* 选择EXTI3做外部中断输入 */
	LL_EXTI_SetEXTISource(LL_EXTI_CONFIG_PORTA,LL_EXTI_CONFIG_LINE3);	//key_dec
	//LL_EXTI_InitTypeDef EXTI_InitStruct;
	/* 选择EXTI3 */
